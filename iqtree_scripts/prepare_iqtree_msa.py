...
def fix_alignment_names(input_fasta, output_fasta, isolate_list, outgroup_list=[], retain_refound = True):
    ...
    # additionally, if any duplicates entries for the same isolate are found, remove them
    isolates_seen = set()
    ...
    # if the isolates seen are all present in the outgroup list, remove the output file
    # also remove it if it has less than 3 sequences
    if all([iso in outgroup_list for iso in isolates_seen]) or len(isolates_seen) < 3:
...
def main():
    ...
    for msa_file in os.listdir(args.input):
        ...
        fix_alignment_names(input_msa,output_msa,name_list,args.outgroup_list)
