...
# current issues:
# 1) splicing introns from the gff changes the coordinates of all downstream features
# this makes the gff no longer "in sync" with the fasta file, so removing all of the recorded introns does not work
# 2) it is possible that some introns overlap, which would greatly complicate removal
        ...
        for fname in os.listdir(gff_input_dir):
            ...
            with open(os.path.join(gff_output_dir,f'{isolatename}.gff3'),'w') as fh_out:
                ...
                for scaffold in sorted(list(gff_db.seqids())):
                    ...
                    # splice all introns from from the CDS features on this scaffold
                    cds_list,intron_regions,retained,removed,spliced,old_cds_list = splice_introns_from_cds_list(cds_list)
                    retained_count += retained
                    removed_count += removed
                    spliced_count += spliced
                    # remove the intron regions from the corresponding fasta record
                    if len(intron_regions) > 0:
                        ...
                        cds_sequence_verifier(old_cds_list,old_fasta_record,cds_list,new_fasta_record.seq)
            ...
            total_genes_processed = removed_count + spliced_count + retained_count 
            _ = fh_log.write(
                f'Resolved {isoform_count} genes with multiple mRNA features. Then, spliced {spliced_count} genes with introns, ' 
                f'removed {removed_count} genes with multiple introns, and retained {retained_count} genes without introns '
                f'of {total_genes_processed} total genes in {isolatename}\n'
                )
...
def cds_sequence_verifier(old_cds_list,old_fasta_record,new_cds_list,new_fasta_record):
    ...
    for ind in range(len(old_cds_list)):
        ...
        if old_cds_seq != new_cds_seq:
            print(f'New CDS sequence does not match old CDS sequence for {old_cds.id}!')
            quit(1)
...
def splice_introns_from_cds_list(cds_list):
    ...
    intron_regions = []
    retained, removed, spliced = 0, 0, 0
    old_cds_list = cds_list.copy()
    for cds_l_index in range(len(cds_list)):
        ...
        # elif len(cds_l) == 2:
            ...
            ####
            # for each pair of adjacent cds features, find the intron between them and remove it
            ###### attempt 1
            ...
            while len(cds_l) > 1:
                ...
                intron_start,intron_end,intron_length = intron_finder(upstream_cds,downstream_cds)
                # add this intron to the list to remove from the fasta file
                intron_regions.append((intron_start,intron_end))
                ...
                cds_l = cds_list[cds_l_index]
                print(cds_l)
    return(cds_list,intron_regions,retained,removed,spliced,old_cds_list)
...
def main():
    ...
    parser.add_argument(
        '--introns','-i',options=['remove','splice'],type=str,
    ...
    if args.introns == 'remove':
        ...
        fasta_introns_removed = os.path.join(args.output,'nucleotide_fasta_NoIntrons')
...
if __name__ == '__main__':
    ...
    logfile = 'test/intron_removal_log.txt'
    splice_introns(gff_input,gff_introns_removed,nucl_fasta_input,fasta_introns_removed,logfile)
    #main()